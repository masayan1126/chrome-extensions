...
> 🚨 **正しい順序**: ストア掲載情報の更新は **必ず zip アップロード前** に行ってください。
> zip をアップロードすると審査キューに入り、その間ストア掲載情報は **編集ロック** されます。
> 一度審査が始まると、説明文の更新は審査完了（数日）まで待たないとできません。
>
> 毎リリース時のチェックリストは [release-checklist.md](./release-checklist.md) を参照。
